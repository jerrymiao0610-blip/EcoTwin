...
function RangeControl({ label, value, min, max, step = 1, unit, onChange }: { label: string; value: number; min: number; max: number; step?: number; unit: string; onChange: (value: number) => void }) {
  ...
  return <label className="range-control"><span>{label}</span><span className="range-value"><input aria-label={`${label} numeric value${unit ? ` in ${unit}` : ""}`} type="number" value={value} min={min} max={max} step={step} onChange={(e) => onChange(Number(e.target.value))} />{unit}</span><input aria-label={label} aria-valuetext={valueText} type="range" value={value} min={min} max={max} step={step} onChange={(e) => onChange(Number(e.target.value))} /></label>;
}
...
export function ClassroomControls({ config, onChange, onReset }: Props) {
  return <section className="controls-panel" aria-labelledby="configuration-title">
    <div className="panel-heading"><div><span className="eyebrow">01 · Configure</span><h2 id="configuration-title">Classroom configuration</h2></div><button type="button" className="reset-button" onClick={onReset}><span aria-hidden="true">↺</span> Reset defaults</button></div>
